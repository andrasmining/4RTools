...
$text = ReadText $path
if (-not $text.Contains('using _4RTools.Model.Vanilla;')) {
...
$text = ReadText $path
if (-not $text.Contains('new KeyChoice(0, "None")')) {
    $old = @'
...
if (-not $text.Contains('private void Guard(System.Action action)')) {
...
$text = ReadText $path
if (-not $text.Contains('VanillaTemporaryActionTests.cs')) {
...
$text = ReadText $path
if (-not $text.Contains('failed += VanillaTemporaryActionTests.Run();')) {
...
'@
if (-not $text.Contains('The Vanilla workspace is now the primary product surface.')) {
    $old = @'
