...
- Cart transfer safety uses the verified Cart weight after every accepted drag. Above 95% full, unknown-weight item categories must stop without a blind full-stack transfer. Current precision-fill rules are Use/Mastela Fruit = 3 weight per item and Etc/Peco Feather = 1 weight per item; Equip stays unknown. Use the positively recognized quantity dialog to request only a capacity-safe amount, verify the resulting Cart-weight delta is coherent with that unit weight, and never exceed the fixed 10000 capacity.
- Cart 100% is a farming milestone: when SMTP/mail alerts are configured, send one Cart-full mail and continue farming on the character. When Cart is still 100% and carried weight reaches at least 50%, send the dedicated Weight Autobattle STOP hotkey, keep that character on an intentional completed-farming hold so recovery does not restart it, and send one DONE mail. Clearing Weight/Cart holds explicitly clears this completed-farming hold too.
