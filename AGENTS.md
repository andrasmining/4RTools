...
Weight-triggered Cart maintenance must remain read-only-memory + ordinary UI input. Reuse the shared fleet reader for verified weight and movement; do not inspect/write inventory memory. Inventory/Cart/category interaction must be dynamically detected from the current client image, never hard-coded desktop coordinates or fixed/percentage category-tab coordinates. Do not add randomized cursor positions or randomized timing for anti-cheat/bot-detection evasion; robustness must come from detected control bounds, deterministic safe interior retry points, and bounded waits driven by observed UI state. Before opening Inventory/Cart, send a dedicated configurable Weight Autobattle STOP hotkey (default Alt+3) after the affected character owns the serialized input lease. Never reuse the character ResumeHotkey as the STOP command. Detect the actual Use/Equip/Etc tab rail from panel/slot/separator structure, positively verify the selected tab after clicking its detected bounds, and require two fresh detected-grid scans with no occupied slot before declaring a category empty and advancing. Cart drops require a positively detected empty Cart slot; never fall back to an arbitrary/calculated drop point. `Enter` is authorized only after a freshly and positively detected quantity dialog; quantity-one transfers have no dialog and receive no Enter. Weight/Cart progress must be visible live in the Recovery log as well as the global debug log. Any ambiguous UI, failed transfer/progress, or apparently full Cart fails closed with Autobattle OFF and a manual hold for only that character until explicitly cleared. Successful maintenance resumes through the shared verified per-character ResumeHotkey/X-Y path and then minimizes.
